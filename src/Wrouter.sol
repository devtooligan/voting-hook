...
contract Wrouter {
    ...
    WrappedToken immutable wrappedToken0;
    WrappedToken immutable wrappedToken1;
    ...
    function _unlockCallbackSwap(CallbackDataSwap memory data) internal returns (bytes memory) {
        ...
        if (deltaAfter0 < 0) {
            ...
            currentWrappedToken = WrappedToken(Currency.unwrap(data.key.currency0));

            // transfer in underlying from user
            currentWrappedToken.token().transferFrom(data.sender, address(this), amount);

            // mint wrapped token for pool
            currentWrappedToken.mint(address(this), amount);

            // data.sender
            data.key.currency0.settle(manager, address(this), amount, data.testSettings.settleUsingBurn);
        }
        if (deltaAfter1 < 0) {
            console.log("b");
            amount = uint256(-deltaAfter1);
            currentWrappedToken = WrappedToken(Currency.unwrap(data.key.currency1));
            ...
            data.key.currency1.settle(manager, address(this), uint256(-deltaAfter1), data.testSettings.settleUsingBurn);
        }
        if (deltaAfter0 > 0) {
            console.log("c");
            amount = uint256(deltaAfter0);
            currentWrappedToken = WrappedToken(Currency.unwrap(data.key.currency0));
            data.key.currency0.take(manager, address(this), amount, data.testSettings.takeClaims);
        ...
        if (deltaAfter1 > 0) {
            ...
            data.key.currency1.take(manager, address(this), amount, data.testSettings.takeClaims);
        }
    ...
    function _unlockCallbackModifyLiquidity(CallbackDataModifyLiquidity memory data) internal returns (bytes memory) {
        ...
        if (delta0 < 0) {
            ...
            data.key.currency0.settle(manager, address(this), amount, data.settleUsingBurn);
        }
        if (delta1 < 0) {
            console.log("2");
            ...
            data.key.currency1.settle(manager, address(this), amount, data.settleUsingBurn);
        }
        if (delta0 > 0) {
            console.log("3");
            amount = uint256(delta0);
            currentWrappedToken = WrappedToken(Currency.unwrap(data.key.currency0));
            data.key.currency0.take(manager, address(this), amount, data.takeClaims);
        ...
        if (delta1 > 0) {
            ...
            amount = uint256(delta1);
            currentWrappedToken = WrappedToken(Currency.unwrap(data.key.currency1));
            data.key.currency1.take(manager, address(this), amount, data.takeClaims);
